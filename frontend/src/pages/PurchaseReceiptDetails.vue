...
<template>
  <!-- Loading State -->
  <div v-if="!receiptResource?.doc || receiptResource?.loading" class="flex items-center justify-center min-h-[60vh]">
    <LoadingIndicator />
  </div>
  ...
  <div v-else>
    ...
    <div class="sticky top-0 z-10 bg-white border-b">
      <div class="flex items-center justify-between p-4">
        <div class="flex items-center gap-4">
          <!-- Back Button -->
          <Button @click="router.push(`/project/${projectResource.doc.name}/invoicing/purchase-receipts`)">
            <template #prefix>
              <FeatherIcon name="arrow-left" class="w-4 h-4" />
            </template>
            <span class="hidden md:inline">Back to Purchase Receipts</span>
          </Button>
          ...
          <div class="flex flex-col">
            ...
            <p class="text-sm text-gray-600 hidden md:inline">
              Created on {{ formatDate(receiptResource.doc.creation) }} by {{ receiptResource.doc.owner }}
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <!-- Status Badge -->
          <Badge
            :variant="getStatusVariant(receiptResource.doc.status) === 'gray' ? 'solid' : 'subtle'"
            :theme="getStatusVariant(receiptResource.doc.status)"
            class="cursor-pointer"
            @click="receiptResource.doc.status === 'Received' && (showCancelDialog = true)"
          >
    ...
    <div class="space-y-8 px-6 py-4">
      ...
      <div class="bg-white rounded-lg border shadow-sm">
          ...
          <div class="flex items-start space-x-4">
            ...
            <div class="flex-1 grid grid-cols-2 gap-4">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ receiptResource.doc.party }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-600">Purchase Order</label>
                <p 
              ...
              <div>
                <label class="text-sm font-medium text-gray-600">Delivery Note Ref</label>
                <p class="mt-1 text-sm text-gray-900">{{ receiptResource.doc.supplier_delivery_note }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-600">Date</label>
                <p class="mt-1 text-sm text-gray-900">{{ formatDate(receiptResource.doc.date, true) }}</p>
              </div>
              
              <!-- Show remarks if cancelled -->
              <div v-if="receiptResource.doc.status === 'Cancelled'" class="col-span-2">
                <label class="text-sm font-medium text-red-600">Cancellation Remarks</label>
                <p class="mt-1 text-sm text-red-600">{{ receiptResource.doc.remarks }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Last Modified -->
        <div v-if="receiptResource.doc.modified_by" class="px-6 py-3 bg-gray-50 text-sm text-gray-600">
          Last modified: {{ formatDate(receiptResource.doc.modified) }} by {{ receiptResource.doc.modified_by }}
        </div>
      </div>
          ...
          <div class="bg-gray-50 border-b min-w-[800px]">
            <div class="flex items-center px-6 py-3">
              <div class="flex-1 grid grid-cols-5 gap-4">
                ...
                <div class="flex items-center gap-2 text-sm font-medium text-gray-700 text-center justify-center">
                ...
                <div class="flex items-center gap-2 text-sm font-medium text-gray-700 text-right justify-center">
          ...
          <div class="divide-y">
            ...
            >
              <div class="flex flex-col items-center text-center max-w-sm">
                ...
                <p class="mt-1 text-sm text-gray-500">This receipt has no items.</p>
              </div>
            </div>
          </div>

          <!-- Save Changes Button -->
          <div v-if="isDraft && hasChanges" class="flex justify-end px-6 py-4 bg-gray-50 border-t">
            <Button
              variant="solid"
              :loading="isSaving"
              :disabled="isSaving"
      ...
      <div v-if="receiptResource.doc.signed_delivery_note" class="bg-white rounded-lg border shadow-sm">
  ...
  <Dialog
    ...
    :options="{
      title: 'Mark Receipt as Received',
      size: 'sm',
      actions: [
        {
          label: 'Confirm',
          ...
          >
            <div class="flex flex-col items-center justify-center space-y-2">
              <div v-if="!file">
                ...
                <div class="text-sm font-medium text-gray-900">Upload Signed Delivery Note</div>
...
const isDraft = computed(() => 
  receiptResource.value?.doc?.status === 'Draft'
)
...
const changedItems = computed(() => {
  ...
  return receiptResource.value.doc.items.filter(item => {
    const original = originalItems.value.find(i => i.name === item.name)
    return original && item.received_quantity !== original.received_quantity && item.received_quantity > 0
  })
})
...
function initializeReceiptResource() {
  if (route.params.receiptId) {
    ...
    watch(() => receiptResource.value?.doc?.items, (items) => {
      if (items) {
...
watch(() => route.params.receiptId, (newId) => {
  if (newId) {
