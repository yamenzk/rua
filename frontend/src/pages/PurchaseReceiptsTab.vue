    ...
    <div v-else class="overflow-x-auto min-h-[60vh]">
      ...
      <div class="divide-y">
              ...
              >
                <div class="flex items-center px-6 py-3">
                  <div class="flex-1 grid grid-cols-6 gap-4">
                    ...
                    <div class="flex flex-col col-span-2">
                          <div class="flex items-center gap-2">
                            <Avatar
                              ...
                              shape="circle"
                            />
                            <span class="text-sm text-gray-900">{{ receipt.party }}</span>
                          </div>
                    ...
                    <div v-if="receipt.supplier_lpo_ref" class="text-sm text-gray-400 flex items-center">
                    ...
                    <div class="flex items-center">
                      <Badge
                        :variant="getStatusVariant(receipt.status) === 'gray' ? 'solid' : 'subtle'"
                    ...
                    <div class="flex items-center">
                      <div 
                        v-if="receipt.status === 'Received' && receipt.signed_delivery_note" 
                        class="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800"
                        @click.stop="openSignedDeliveryNote(receipt.signed_delivery_note)"
                      >
                        <FeatherIcon name="file-text" class="w-4 h-4" />
                        View Signed Note
                      </div>
                      <div 
                        v-if="receipt.status === 'Cancelled' && receipt.remarks" 
        ...
        >
          ...
          <p class="text-sm text-gray-600">There are no purchase receipts created yet.</p>
        </div>
      </div>
    </div>
  </div>
...
const filteredReceipts = computed(() => {
  return purchaseReceiptResource.data?.filter(receipt => 
    receipt.project === props.projectResource.doc?.name
  ) || []
})
