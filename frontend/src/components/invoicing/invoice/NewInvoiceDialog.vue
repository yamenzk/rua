        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-4">
            <div v-for="party in availableParties" :key="party.name" 
              ...
              :class="{'border-gray-900 bg-gray-100': formData.party === party.name}"
...
import { Dialog, Switch, Checkbox, FormControl, FeatherIcon } from 'frappe-ui'
import { formatCurrency, getServerDate } from '@/utils/format'
import ReviewInvoiceDialog from '@/components/invoicing/invoice/ReviewInvoiceDialog.vue'
...
const availableParties = computed(() => {
  ...
  return parties.filter(party => party.type.toLowerCase() === 'client')
})
...
const formData = ref({
  date: getServerDate(),
  party: null, // No default party, must be selected
  type: '',
  amount: null,
...
const isFormValid = computed(() => {
  return formData.value.date && 
         ...
         formData.value.party && // Add party validation
         !amountError.value &&
...
function selectParty(party) {
  formData.value.party = party.name
}
...
function resetForm() {
  formData.value = {
    date: getServerDate(),
    party: null, // Reset party selection
    type: '',
    amount: null,
    is_vat_inclusive: false,
    retention_enabled: false,
