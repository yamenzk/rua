...
export function isWithinRange(date, startDate, endDate) {
	...
	return (
		(checkDate.isAfter(start, 'day') || checkDate.isSame(start, 'day')) &&
...
export function getStatusTheme(status) {
	...
	switch (lowerStatus) {
		case 'draft':
			return 'orange' // Or 'gray', 'yellow' depending on preference
		case 'submitted': // Example if you add this status later
			return 'blue'
		case 'final':
			return 'green' // Or 'gray' if 'final' isn't necessarily "good"
		case 'cancelled':
			return 'red'
		case 'rejected': // Example
			return 'red'
		default:
...
export function getFileIcon(url) {
	if (!url) return 'file' // Default icon if no URL
	try {
		...
		const iconMap = {
			...
			rtf: 'file-text',
			// Spreadsheets
			xls: 'grid', // Using grid as a proxy for spreadsheet
			xlsx: 'grid',
			ods: 'grid',
			csv: 'grid',
			// Presentations
			ppt: 'monitor', // Using monitor as a proxy
			pptx: 'monitor',
			odp: 'monitor',
			// Images
			jpg: 'image',
		...
		return iconMap[extension] || 'file' // Default to 'file' if extension not mapped
	} catch (error) {
